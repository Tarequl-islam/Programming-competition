...
int main(){ //s: 02.40 am - e: 0.00am;
    ...
    while (t--){
        ...
        vector<vector<int>> poss(2000010);
        for (int i = 0; i < n; ++i) {
            int x = a[i];
            int cur = 0;
            while (x > 0) {
                poss[x].push_back(cur);
                x /= 2;
                cur++;
            }
        }
        ...
        for (int i=0; i<=200000; i++) {
            ...
            if (int(poss[i].size()) < k) continue;
            ans = min(ans, accumulate(poss[i].begin(), poss[i].begin() + k, 0));
        }
        cout << ans << endl;
    }
    return 0;
